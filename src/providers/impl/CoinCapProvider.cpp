    ...
    , name_(name) {
        for (const auto& s : config_caps) {
            ...
            if (crypto_cap.has_value()) {
                ...
                Logger::Warn("[CoinCap] Unknown capability '{}' in config", s);
            }
        }
    }
...
std::optional<double> CoinCapProvider::GetCryptoPrice(const std::string& ticker) {
    if (!HasCapability(CryptoCapability::RealtimePrice)) {
        return std::nullopt;
    }

    auto json_opt = PerformGet("/assets/" + ticker);
...
std::optional<CryptoAsset> CoinCapProvider::GetCryptoAssetInfo(const std::string& ticker) {
    ...
    try {
        ...
        Logger::Error("[CoinCap] Parse AssetInfo error for {}: {}", ticker, ex.what());
        return std::nullopt;
    }
}

std::vector<CryptoAsset> CoinCapProvider::GetCryptoTopList(const int limit) {
    if (!HasCapability(CryptoCapability::TopList)) {
        return {};
    }

    auto json_opt = PerformGet("/assets", {{"limit", std::to_string(limit)}});
    ...
    try {
        ...
        Logger::Error("[CoinCap] TopList parse error: {}", ex.what());
    }
    return results;
}

std::vector<CryptoPriceCandle> CoinCapProvider::GetCryptoHistory(const std::string& ticker, 
    ...
    auto json_opt = PerformGet("/assets/" + ticker + "/history", {
    ...
    try {
        ...
        Logger::Error("[CoinCap] History parse error for {}: {}", ticker, ex.what());
    }
    return results;
}

std::vector<CryptoAsset> CoinCapProvider::SearchAsset(const std::string& query) {
    if (!HasCapability(CryptoCapability::TopList)) {
        return {};
    }

    auto json_opt = PerformGet("/assets", {{"search", query}, {"limit", "10"}});
    ...
    try {
        ...
        Logger::Error("[CoinCap] Search parse error: {}", ex.what());
    }
    return results;
}

std::optional<GlobalCryptoMetrics> CoinCapProvider::GetGlobalMetrics() {
    if (HasCapability(CryptoCapability::GlobalMetrics)) {
        Logger::Warn("[CoinCap] GlobalMetrics requested but not natively supported by API v2");
    }
    return std::nullopt;
}

std::optional<OrderBook> CoinCapProvider::GetOrderBook(const std::string&, int) {
    if (HasCapability(CryptoCapability::OrderBook)) {
        Logger::Warn("[CoinCap] OrderBook requested but only available via WebSocket");
    }
    return std::nullopt;
}
